...
- **实验样例**：使用 8 个确定性选择的匿名 Validation 用户，展示历史、离线正例、Text-only Top-10、匹配词与产品分流建议；平衡样例不代表总体命中率。
- **创建模拟用户**：从 360 条匿名训练目录中选择 1–6 条历史视频，按正式实验同一画像公式对完整 1,305 个冷候选实时排序，并展示每条推荐主要关联的历史；也可从 6 类共 58 个可识别词中点选或输入英文兴趣词做快速近似。没有未来行为标签，因此不显示命中率，也不写回实验结果。
...
浏览器打开 `http://localhost:3000/`。本地数据文件被 Git 忽略；页面不读取原始 CSV、封面、视觉特征或 Test。历史目录是主题平衡的机制演示样本，不代表真实内容分布；“主要相关历史”是文本相似度解释，不是因果归因。当前冻结 tokenizer 只可靠识别英文/数字兴趣词，不具备中文语义理解。详细边界见 [Demo 协议](docs/DEMO_PROTOCOL.md)。
